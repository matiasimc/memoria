...
El siguiente paso es agrupar las restricciones sobre la misma variable de tipo, usando las reglas del teorema \ref{teo1}. Aplicando el algoritmo \ref{pseudogroup}, el conjunto de restricciones del ejemplo \ref{ejsimplify} se reduce a solo dos restricciones, lo que se muestra en el ejemplo \ref{ej3-2}.
...
\begin{algorithm}\captionsetup{labelsep=newline}
  \centering
  \caption{Agrupación de restricciones}
  \label{pseudogroup}
    \begin{algorithmic}[1]
  ...
  \label{pseudoperations}
    \begin{algorithmic}[1]
      \Function{performOperations}{\texttt{cs}}
          \For {\texttt{constraint \textbf{in} cs}}
            \If {\texttt{constraint.right \textbf{is} $\sqcap$ \textbf{and} constraint.right.isConcrete()}}
