...
Acá, se quiere inferir que el método \texttt{login} tiene a \texttt{Top} como faceta de desclasificación, debido a la aplicación de la regla \texttt{TmH} de \textit{type-based declassification}.
    ...
    \begin{algorithmic}
      \Function{Constraint\_Generator}{node, pc}
          ...
          \If {node is MethodInvocation}
            ...
            \For {argument$\gets$ callArguments}
  ...
  \begin{mdframed}
    \begin{algorithmic}
      \Function{performOperations}{cs}
          \For {c in cs}
            \If {c.right is MeetType and c.right.isConcrete()}
              \State c.right $\gets$ meet(c.right)
            \EndIf
            \If {c.left is JoinType and c.left.isConcrete()}
